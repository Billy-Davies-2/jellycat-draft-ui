...
// TestAddPlayerRandomCuddlePointsBounds tests that AddPlayer assigns random cuddle points
// within the 25-79 range when CuddlePoints is not set
func TestAddPlayerRandomCuddlePointsBounds(t *testing.T) {
	dal := NewMemoryDAL()

	// Test multiple times to ensure randomization works correctly
	for i := 0; i < 50; i++ {
		player := &models.Player{
			Name:     fmt.Sprintf("Random Player %d", i),
			Position: "CC",
			Team:     "Test",
			Points:   100,
			// CuddlePoints intentionally not set (defaults to 0)
			Tier: models.TierA,
		}

		addedPlayer, err := dal.AddPlayer(player)
		if err != nil {
			t.Fatalf("AddPlayer() failed for iteration %d: %v", i, err)
		}

		// Verify the cuddle points are within the expected range [25, 79]
		if addedPlayer.CuddlePoints < 25 {
			t.Errorf("Iteration %d: cuddle points %d is below minimum 25", i, addedPlayer.CuddlePoints)
		}
		if addedPlayer.CuddlePoints > 79 {
			t.Errorf("Iteration %d: cuddle points %d is above maximum 79", i, addedPlayer.CuddlePoints)
		}
	}
}
